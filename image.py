...
def SpreadSpectrumEmbed(origfile, markedfile, watermark, alpha):
    ...
    # 为了方便最后统合所有算法，在这里强制规定了加密位置和大小关系
    bigger_x = 2
    bigger_y = 4
    more_number = 1
    ...
    for c in bindata:
        if int(c) == more_number:
            ...
            if block[index][bigger_y][bigger_x] - block[index][bigger_x][bigger_y] < alpha:
                block[index][bigger_y][bigger_x] = (block[index][bigger_y][bigger_x] + block[index][bigger_x][
                    bigger_y]) / 2 + alpha
                block[index][bigger_x][bigger_y] = (block[index][bigger_y][bigger_x] + block[index][bigger_x][
                    bigger_y]) / 2 - alpha
        else:
            if block[index][bigger_y][bigger_x] > block[index][bigger_x][bigger_y]:
                block[index][bigger_y][bigger_x], block[index][bigger_x][bigger_y] = block[index][bigger_x][
...
def decode_get_watermark(makefile, more_number, bigger_y, bigger_x):
    ...
    watermark = ""
    temp_code = bin(0)[2:0]
    for i in range(0, transform_code(length)):
...
if __name__ == "__main__":
    more_number, bigger_y, bigger_x = SpreadSpectrumEmbed('lena.jpg', 'lena-a.jpg', 'hello', 1)
    decode_get_watermark('ok.jpg', more_number, bigger_y, bigger_x)
